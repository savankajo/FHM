...
export async function PUT(request: Request) { const session = await getSession(); if (!session) return NextResponse.json({error:'Unauthorized'},{status:401}); const body=await request.json(); const preferences={reminders:body.reminders!==false,invitations:body.invitations!==false,messages:body.messages!==false,live:body.live!==false,media:body.media!==false}; await prisma.user.update({where:{id:session.userId},data:{notificationPreferences:preferences}}); return NextResponse.json({preferences}); }
