...
export default async function TeamDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="team-detail-page">
    <header className="page-header"><Link href="/messages" className="page-back-btn" aria-label="Back to Messages"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M15 18l-6-6 6-6" /></svg></Link><div><h1 className="page-title">{team.name}</h1><p className="page-kicker">Your ministry team</p></div></header>
    ...
    <div className="team-chat-btn-wrap"><Link href={`/messages/team/${team.id}`} className="team-chat-btn">Message Team</Link></div>
  </main>;
}
